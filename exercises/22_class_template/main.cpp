...
struct Tensor4D {
    ...
    Tensor4D &operator+=(Tensor4D const &others) {
        ...
        for (unsigned int i = 0; i < size; i++) {
            unsigned int idx = 0, offset_this = 1, offset_others = 1;
            for (int dim = 3; dim >= 0; dim--) {
                unsigned int pos = (i / offset_this) % shape[dim];
                if (others.shape[dim] != 1) {
                    idx += pos * offset_others;
                } else {
                    idx += 0;
                }
                offset_this *= shape[dim];
                offset_others *= others.shape[dim];
            }
